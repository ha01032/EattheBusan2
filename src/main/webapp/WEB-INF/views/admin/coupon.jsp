	...
	<div id="content">
		...
		<div id="list">
			...
			<table id="example" class="display" style="width: 100%">
				...
				<tbody>	
					<c:forEach var="dto" items="${couponList}" begin="0"
						step="1">
						<tr id="couponClick">
							<th id="sendCouponCode">${dto.couponCode}</th>
							<th>${dto.foodCode}</th>
							 <th>${dto.couponName}</th>
							<th>
						 	${dto.couponStartdate} ~ ${dto.couponEnddate}
								<%-- <fmt:parseDate value="${dto.couponStartdate}" var="startDate" pattern="yyyy-MM-dd" />
								<fmt:formatDate value="${startDate}" pattern="yyyy년  MM월  dd일" /> ~ 
								<fmt:parseDate value="${dto.couponEnddate}" var="endDate" pattern="yyyy-MM-dd " />
								<fmt:formatDate value="${endDate}" pattern="yyyy년  MM월  dd일 " /> --%>
							</th>
							<th>${dto.couponSalerate}</th>
							<th>${dto.couponCostsale}</th>
							<th>${dto.couponStatus}</th>
						</tr>
					</c:forEach> 
				</tbody>
			</table>
				...
				<form action="${root}/coupon/couponUpdateOk.go" method="post"
					...
					<div class="coupon">
						...
						<div class="btn">
							...
							<input class="listBtn" type="button" id="foodDeleteBtn" value="삭제하기"/>	
						</div>
					</div>
				</form>
			</div>
